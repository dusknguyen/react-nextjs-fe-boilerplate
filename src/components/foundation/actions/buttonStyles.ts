...
/** Supported button corner treatments. */ export type ButtonShape = 'pill' | 'rounded'; /** Supported button control sizes. */
/** Public contract for button size. */
export type ButtonSize = 'large' | 'medium' | 'small'; /** Supported button surface treatments. */
/** Public contract for button variant. */
export type ButtonVariant = 'ghost' | 'outline' | 'solid';
const base = 'relative shrink-0 self-start flex-row items-center justify-center gap-2 overflow-hidden border web:transition-all web:duration-200 web:hover:-translate-y-0.5';
